# pavlusha_1.py - PEREUSTANOVKA + REAL TIME LOG
...
if r.status_code == 200:
    ...
    for line in process.stdout:
        ...
        # Если прошло больше 90 сек с последней строки — что-то зависло
        if time.time() - last_line_time > 90:
            process.kill()
            fail("Установщик завис (нет вывода > 90 сек)")
            print("\n  Возможные причины:")
            print("    1. PyPI заблокирован в РФ")
            print("    2. Нужен VPN для установки requests")
            print("  Решение: включите VPN и перезапустите.")
            os.remove(ipath)
            exit()
